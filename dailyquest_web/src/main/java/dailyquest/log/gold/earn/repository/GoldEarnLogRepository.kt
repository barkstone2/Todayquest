...
interface GoldEarnLogRepository: JpaRepository<GoldEarnLog, Long> {
    ...
    @Query("select coalesce(sum(gel.amount), 0) from GoldEarnLog gel where gel.user.id = :userId")
    fun getTotalGoldEarnOfUser(@Param("userId") userId: Long): Int

    @Query("select coalesce(sum(gel.amount), 0) from GoldEarnLog gel where gel.user.id = :userId and gel.source = :source")
