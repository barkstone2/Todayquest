...
interface PreferenceQuestRepository : JpaRepository<PreferenceQuest, Long> {
    ...
    @Query("select count(q) from PreferenceQuest pq left join Quest q on q.preferenceQuest.id = pq.id where pq.user.id = :userId and pq.deletedDate is null group by pq.id")
